# How the v3 (Electron) build is put together
...
`fractal-app-lite` (in `../src/backend/` and `../src/fractal_lite/`, at the repo root) is a Python application.
...
The frontend source lives in `../src/frontend/` (repo root).
The compiled output lands in `../src/frontend/build/`.
...
2. **Building the SvelteKit frontend**: SvelteKit is a Node.js tool. `npm ci`
   and `npm run build` inside `../src/frontend/` (repo root) compile the
   Svelte components into static files.
...
Step 1: Build the SvelteKit frontend
        (scripts/build-frontend.sh — npm ci + npm run build inside ../src/frontend/)
...
The SvelteKit source in `../src/frontend/` (repo root) is compiled into plain
HTML/CSS/JS files. This is a standard Node.js build:
...
shallow-clones it locally into `../fractal-web-clone/` (repo root) and
`vite.config.js` tells the Vite bundler to resolve the name
`fractal-components` to that local clone.
...
`node_modules/`, regardless of where the importing code lives." We don't want
to commit this change permanently, so `build-frontend.sh` applies the patch
temporarily just for the duration of the build, then restores the original
file.
...
written to a temporary directory outside the repo, so the working tree is
never modified.
...
## 5. One repo, three branches

`fractal-app-lite`'s Python/SvelteKit source (`src/backend/`, `src/fractal_lite/`,
`src/frontend/`, at the repo root — one level up from this `electron/` folder)
is shared across the repo's release tracks, one per branch: `v1` (run from
source), `v2` (PyInstaller single-file executable), and `v3` (this Electron
wrapper). There is no submodule — the Electron tooling (this whole `electron/`
folder: `src/main/`, `src/preload/`, `package.json`, `electron-builder.yml`,
`scripts/`) simply lives alongside the Python/SvelteKit source in the same
checkout.

`fractal-web-clone/` at the repo root (`../fractal-web-clone/` from here) is
*not* a submodule either. It is a plain directory created by
`build-frontend.sh` and listed in `.gitignore`. If you delete it, the next run
of `build-frontend.sh` will re-clone it.

---

## 6. The CI pipeline

The GitHub Actions workflow (`.github/workflows/release.yml`) runs on a push of
a `v3.x.y` tag, building on five machines in parallel, one per platform:

| Runner            | Produces                          |
|-------------------|-----------------------------------|
| ubuntu-22.04      | Linux AppImage (x64)              |
| macos-26          | macOS dmg (arm64, Apple Silicon)  |
| macos-26-intel    | macOS dmg (x64, Intel)            |
...
```
1. Checkout
2. Set version — extract version number from the git tag (e.g. v3.1.5 → 3.1.5)
...
**Why five separate machines and not cross-compilation?**
...
- **On a `v3.x.y` tag push** (e.g. `git tag v3.1.5 && git push --tags`): the
  workflow runs and uploads the five distributables to a GitHub Release. This
...
Source code (git)
│
├── ../src/                 (repo root — shared with v1/v2)
...
All paths under `resources/`, `out/`, and `dist-electron/` above are relative
to this `electron/` folder — that's where PyInstaller output, compiled
TypeScript, and the final installers actually land.
