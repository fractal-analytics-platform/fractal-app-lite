...
This is the `electron/` subfolder of the `v3` branch of **fractal-app-lite**, which wraps the Python web application in [Electron](https://www.electronjs.org/), giving it a native window instead of a browser tab.

The Python/SvelteKit application lives at the repo root, one level up, in `../src/` (`backend/`, `fractal_lite/`, `frontend/`) — the same source used by the other branches (`v1`: run from source; `v2`: packaged with PyInstaller alone). This `electron/` folder holds the Electron wrapper: `src/main/`, `src/preload/`, and the npm/electron-builder tooling. Before the app can be distributed (or run in dev mode), a build step packages the Python code into a self-contained binary using PyInstaller, and places it under `electron/resources/`.
...
The Python source is organized as (one level up, at the repo root):
```
../src/
...
```
fractal-app-lite/  (v3 branch)
│
├── src/                        Shared with v1/v2 — the Python/SvelteKit application
│   ├── backend/                FastAPI app (routes, state, job runner)
│   ├── fractal_lite/           Core Python library
│   └── frontend/               SvelteKit app (builds to static files)
├── pyproject.toml              Python package + pixi tasks (backend-only dev, see README)
├── fractal-web-clone/          GITIGNORED — cloned by build-frontend.sh
│
└── electron/                   This folder — the Electron wrapper
    ├── src/
    ...
    │
    ├── scripts/
    │   ├── build-frontend.sh   Builds the SvelteKit frontend from ../src/frontend/
...
The SvelteKit frontend imports a component library called `fractal-components`. That library lives in a separate GitHub repo (`fractal-analytics-platform/fractal-web`). The build script reads the pinned version tag from `vite.config.js` and shallow-clones it into `fractal-web-clone/` at the repo root (`../fractal-web-clone/` from here). The vite config then aliases `fractal-components` to the source files in that clone.

**1b. Build the frontend**

Runs `npm ci && npm run build` inside `../src/frontend/` (the repo-root Python/SvelteKit source). SvelteKit with `adapter-static` compiles the Svelte components into plain HTML, JavaScript, and CSS files in `../src/frontend/build/`. These are static files — no server-side rendering, no Node.js needed at runtime.

Because `fractal-web-clone` has no `node_modules` of its own, vite 8's bundler (rolldown) cannot resolve its dependencies at build time. The script temporarily patches `vite.config.js` to add `resolve.dedupe` — forcing those packages to resolve from the frontend's own `node_modules`. It reads the committed file content directly from git, writes the patched version, runs the build, then restores the original with `git checkout`. The working tree is clean before and after.

**1c. Build the PyInstaller binary**

Creates a temporary directory outside the repo and writes `_electron_entry.py` there:
```python
...
The output is copied to `resources/fractal-app-lite/`. The temp directory and any stray `.spec` files are removed by a shell trap on script exit, leaving the working tree clean.
...
- **`fractal-web-clone`**: the `fractal-web-clone/` directory at the repo root is not a git submodule — it is a plain directory created by `build-frontend.sh` and gitignored. If you delete it, the next `build-frontend.sh` run will re-clone it.
