...
def _resolve_task(pkg_name: str, version: str | None, name: str) -> Task:
    ...
    :data:`tasks_registry`, then — failing that — by collecting the matching
    GitHub release for ``pkg_name`` (looked up in the curated package index).
    ...
    for tag in tags_to_try:
        try:
            tasks_registry.collect_from_gitrelease(
                entry.repo_url, tag=tag, overwrite=True
            )
            break
...
class Workflow(BaseModel):
    ...
    def resolve_schemas(self) -> "Workflow":
        """Return a copy with each :class:`Task` step's full definition (incl. the
        ``args_schema_*`` blanked by :func:`strip_task_schemas`) overlaid from the
        :data:`tasks_registry`, matched by ``unique_id``.
    ...
    @classmethod
    def from_fractal_dict(cls, data: dict[str, Any]) -> "Workflow":
        """Reconstruct a workflow from Fractal's workflow-export shape.

        Each entry's task is resolved via :func:`_resolve_task` (registry lookup,
        auto-collecting from the package index when missing); its
        ...
        for entry in data.get("task_list", []):
            ...
            resolved = _resolve_task(
                task_ref["pkg_name"], task_ref.get("version"), task_ref["name"]
            )
            steps.append(
                resolved.model_copy(
                    update={
    ...
    @classmethod
    def from_fractal_json(cls, data: str) -> "Workflow":
        """Reconstruct a workflow from a Fractal workflow-export JSON string."""
        return cls.from_fractal_dict(json.loads(data))
